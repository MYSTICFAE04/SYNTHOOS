...
describe("AssistantMessageItem", () => {
  ...
  describe("assistant messages", () => {
    it("should render assistant label with SynthoOS icon", () => {
      const message = createMessage({
        role: "assistant",
      ...
      expect(screen.getByText("SynthoOS Assistant")).toBeInTheDocument();
      expect(screen.getByAltText("SynthoOS Assistant")).toBeInTheDocument();
    });
  });
  ...
  describe("plain text response", () => {
    it("should render markdown for regular text content", () => {
      const message = createMessage({
        role: "assistant",
        content: "SynthoOS is a visual flow builder.",
        status: "complete",
      });
      ...
      expect(screen.getByTestId("markdown-content")).toHaveTextContent(
        "SynthoOS is a visual flow builder.",
      );
    });
    ...
    it("should_render_markdown_when_qa_response_contains_example_component_code", () => {
      ...
      const qaWithExampleCode = [
        ...
        "```\n\n",
        "Save the file and restart SynthoOS.",
      ].join("");
