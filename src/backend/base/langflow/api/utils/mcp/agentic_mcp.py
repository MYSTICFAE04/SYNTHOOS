"""Utilities for auto-configuring the SynthoOS Agentic MCP server."""
...
async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    """Auto-configure the SynthoOS Agentic MCP server for all users.
    ...
    await logger.ainfo("Auto-configuring SynthoOS Agentic MCP server for all users...")
    ...
    try:
        ...
        server_config = {
            ...
            "metadata": {
                "description": "SynthoOS Agentic MCP server providing tools for flow/component operations, "
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the SynthoOS Agentic MCP server from all users.
    ...
    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing SynthoOS Agentic MCP server from all users...")
