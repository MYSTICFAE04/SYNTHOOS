...
async def verify_project_auth(
    ...
    # OAuth projects must present a valid API key at the SynthoOS transport endpoint: network-level
    ...
    # composer-to-SynthoOS hop should be authenticated explicitly once mcp-composer can forward
...
async def install_mcp_config(
    ...
    try:
        ...
        if use_mcp_composer:
            ...
            command = "uvx"
            args = ["mcp-proxy"]
            # Check if we need to add SynthoOS API key headers
            # Necessary only when Project API Key Authentication is enabled

            # Generate a SynthoOS API key for auto-install if needed
        ...
        # Remove stale entries that point to the same SynthoOS URLs (e.g. after the project is renamed)
...
async def register_project_with_composer(project: Folder):
    ...
    try:
        ...
        if not settings.host or not settings.port:
            error_msg = "SynthoOS host and port must be set in settings to register project with MCP Composer"
            raise ValueError(error_msg)
