...
try:
    ...
    lfx_app = typer.Typer(name="lfx", help="SynthoOS Executor commands")
...
def run(
    *,
    ...
    if use_direct_uvicorn():
        ...
        with progress.step(6):
            # Use Gunicorn with SynthoOSUvicornWorker for non-Windows systems
            from langflow.server import SynthoOSApplication
            ...
            server = SynthoOSApplication(app_factory, options)
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    telemetry_text = (
        (
            "We collect anonymous usage data to improve SynthoOS.\n"
        ...
        if os.getenv("DO_NOT_TRACK", os.getenv("LANGFLOW_DO_NOT_TRACK", "False")).lower() != "true"
            ...
            "We are [bold]not[/bold] collecting anonymous usage data to improve SynthoOS.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_host = get_best_access_host(host, port)
    access_link = f"[bold]{status_icon} Open SynthoOS {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
    ...
    try:
        ...
        fallback_message = (
            ...
            f"{telemetry_text}\n\n"
            f"[OK] Open SynthoOS -> {protocol}://{access_host}:{port}"
        )
        try:
            ...
            logger.info(f"Open SynthoOS: {protocol}://{access_host}:{port}")
