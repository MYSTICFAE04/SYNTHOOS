...
class MCPToolsComponent(ComponentWithCache):
            ...
            try:
                ...
                try:
                    ...
                    if not is_langflow_standalone:
                        raise
                    await logger.ainfo(
                        "SynthoOS package not available; using MCP server config from flow value (LFX standalone mode)."
                    )
                else:
    ...
    def _get_session_context(self) -> str | None:
        """Get the SynthoOS session ID for MCP session caching."""
