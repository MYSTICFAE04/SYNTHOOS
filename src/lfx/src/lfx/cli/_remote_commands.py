...
def register(app: typer.Typer) -> None:
    ...
    @app.command(
        name="status", help="Compare local flow files against a remote SynthoOS instance", rich_help_panel="Remote"
    )
    def status_command_wrapper(
        ...
        target: str | None = typer.Option(
            None,
            "--target",
            help="SynthoOS instance URL (inline override — skips config file lookup).",
        ),
        api_key: str | None = typer.Option(
            None,
            "--api-key",
            help="API key for the SynthoOS instance (used with --target or to override config).",
        ),
    ...
    @app.command(
        name="push",
        help="Push flow JSON to a remote SynthoOS instance (upsert by stable ID)",
    ...
    def push_command_wrapper(
        ...
        """Push SynthoOS flows to a remote instance using stable IDs for upsert (lazy-loaded)."""
    ...
    @app.command(
        name="pull", help="Pull flows from a remote SynthoOS instance to local files", rich_help_panel="Remote"
    )
    def pull_command_wrapper(
        ...
        """Pull and normalize flows from a remote SynthoOS instance (lazy-loaded)."""
    ...
    def export_command_wrapper(
        ...
        """Export and normalize SynthoOS flow JSON for version control (lazy-loaded)."""
