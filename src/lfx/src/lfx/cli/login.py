"""lfx login -- validate credentials against a remote SynthoOS instance.
...
def _probe_connection(client: Any, sdk: Any) -> tuple[bool, str, int]:
    ...
    try:
        ...
        # local SynthoOS version exactly.  Treat this as a successful probe.
    ...
    *,
    ...
    if not ok:
        if msg == "auth":
            ...
            if key_env_name:
                err_console.print("  1. Open SynthoOS → Settings → API Keys → Create a new key")
        ...
        if msg.startswith("connection:"):
            ...
            err_console.print()
            err_console.print("[bold]How to fix:[/bold]")
            err_console.print("  • Make sure your SynthoOS instance is running")
