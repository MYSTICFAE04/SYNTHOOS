...
"""Fallback SynthoOS server URL when neither ``--target`` nor ``LANGFLOW_HOST`` is set."""
...
def reload_via_http(
    *,
    ...
    """POST a reload request to the SynthoOS extension dev server.
                ...
                "errors": [
                    {
                        ...
                        "message": f"Could not reach SynthoOS server at {url}: {exc}",
                        "location": url,
                        "hint": (
            ...
            "errors": [
                {
                    ...
                    "location": url,
                    "hint": "Confirm the URL points at a SynthoOS server with the v1 API enabled.",
                }
            ],
        }
