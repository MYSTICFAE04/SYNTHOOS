...
#
# Configure your SynthoOS instances here.
...
#
# Quick start:
#   1. Open SynthoOS → Settings → API Keys → Create a new key
...
_TEST_FLOWS_PY = '''\
"""Integration tests for SynthoOS flows.
...
def test_flow_output_quality(flow_runner):
    ...
    result = flow_runner("my-flow-endpoint", "What is SynthoOS?")
...
_GITIGNORE = """\
# SynthoOS credentials -- never commit API keys
...
def init_command(
    ...
    *,
    ...
    if example:
        ...
        console.print("  5. [bold]lfx push --dir flows/ --env local[/bold]    (deploy to SynthoOS)")
    else:
