...
Commands shipped here:
    ...
    - ``dev``       -- register a local extension and launch SynthoOS with
                       it loaded.
...
extension_app = typer.Typer(
    name="extension",
    help="Author and inspect SynthoOS Extensions.",
...
def validate_command(
    ...
    *,
    ...
    subprocess with a temporary SynthoOS state directory.
    """
...
def reload_command(
    ...
    target: str | None = typer.Option(
        None,
        "--target",
        help=("SynthoOS server URL (default: $LANGFLOW_HOST / http://localhost:7860)."),
    ),
    api_key: str | None = typer.Option(
        None,
        "--api-key",
        help="API key for the SynthoOS server (default: $LANGFLOW_API_KEY).",
    ),
    output_format: str = typer.Option(
        "text",
        "--format",
...
def init_command(
    ...
    *,
    ...
    name: str | None = typer.Option(
        None,
        "--name",
        help=("Human-readable display name shown in SynthoOS.  Defaults to the title-cased extension id."),
    ),
) -> None:
...
@extension_app.command(
    name="dev",
    help="Register a local extension and launch SynthoOS with it loaded.",
)
def dev_command(
    target: str = typer.Argument(
        ".",
    ...
    *,
    ...
    """Register the extension and launch a SynthoOS dev server.
    ...
    AC #4 ("boots SynthoOS with the new Extension visible in the palette
    ...
    typer.echo("  - Click 'Reload' on the bundle header in the SynthoOS palette.")
